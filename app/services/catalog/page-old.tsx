...
export default async function ServicesCatalogPage({
  ...
  // Filter to show only TOP-TIER packages (remove Lean)
        ...
        <div className="container mx-auto max-w-7xl px-4">
            ...
            <div className="mt-12">
              ...
              <div className="mb-8 rounded-2xl border border-white/10 bg-white/[0.02] p-6">
                ...
                <div className="grid gap-4 md:grid-cols-5">
                  ...
                  <div>
                    <label className="text-xs text-white/90 font-semibold uppercase tracking-wide">Price</label>
                    <select className="mt-2 w-full rounded-lg bg-white/10 border border-white/10 text-white text-sm px-3 py-2 hover:bg-white/15 transition">
                  ...
                  <div>
                    ...
                    <select className="mt-2 w-full rounded-lg bg-white/10 border border-white/10 text-white text-sm px-3 py-2 hover:bg-white/15 transition">
                      ...
                      <option value="onetime">One-Time Launch</option>
                    </select>
                  </div>
                  <div>
                    <label className="text-xs text-white/90 font-semibold uppercase tracking-wide">Search</label>
                    <input type="text" placeholder="Service name..." className="mt-2 w-full rounded-lg bg-white/10 border border-white/10 text-white text-sm px-3 py-2 placeholder-white/40 hover:bg-white/15 transition" />
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                {services.map((service) => (
                  <div key={service.id} className="group rounded-3xl border border-white/10 bg-[#0b0f16] p-6 hover:border-white/20 transition">
                    <div className="flex items-start justify-between gap-4 mb-3">
                      <div>
                        <h3 className="text-lg font-bold text-white">{service.name}</h3>
                        <p className="mt-1 text-xs text-white/45 uppercase tracking-wider">{service.category}</p>
                      </div>
                      <service.icon className="h-6 w-6 text-blue-400 flex-shrink-0" />
                    </div>
                    ...
                    <div className="flex items-end justify-between gap-3 border-t border-white/10 pt-4">
                      <div>
                        ...
                        <p className="text-lg font-bold text-white mt-0.5 whitespace-nowrap">{service.price_display}</p>
                      </div>
                      {service.tier && service.tier.length > 0 && (
                        <div className="flex gap-1">
                          {service.tier.includes('top') && (
                            <span className="rounded-lg bg-purple-500/20 border border-purple-500/30 px-2 py-1 text-xs font-semibold text-purple-200">Top</span>
                          )}
                          {service.tier.includes('lean') && (
                            <span className="rounded-lg bg-blue-500/20 border border-blue-500/30 px-2 py-1 text-xs font-semibold text-blue-200">Lean</span>
                          )}
                        </div>
                      )}
                    </div>
          ...
          {(activeView === "all" || activeView === "stacks") && (
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      ...
                      <span className={`rounded-2xl px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.18em] ${
                        preset.tier === 'top' 
                    ...
                    {/* Pricing Summary - Base Pricing */}
                    <div className="mt-5 rounded-2xl border-2 border-white/20 bg-gradient-to-r from-blue-950/40 to-indigo-950/40 p-4">
                      <p className="text-xs font-semibold text-blue-200 uppercase tracking-wider">Base Monthly Investment</p>
                      <div className="mt-2 flex items-baseline gap-2">
                        <p className="text-4xl font-black text-white">
                          $
                          {preset.officeType === 'mayor' 
                            ? preset.tier === 'top' ? '2,995' : '1,495'
                            : preset.tier === 'top' ? '1,995' : '995'
                          }
                        </p>
                      ...
                      <p className="mt-2 text-xs text-white/50">
                        {preset.tier === 'top' 
                          ? 'Full-featured package with all core services + launch + add-ons'
                          : 'Essential package with core services for focused campaigns'
                        }
                      </p>
                    </div>
                ...
                {/* Councillor Packages */}
                <div>
                  <h3 className="text-2xl font-bold text-white mb-4">Councillor Campaigns</h3>
                  ...
                  <div key={preset.id} className="rounded-3xl border border-white/10 bg-[#0b0f16] p-6">
                    ...
                    {/* Pricing Summary - Base Pricing */}
                    <div className="mt-5 rounded-2xl border-2 border-white/20 bg-gradient-to-r from-indigo-950/40 to-purple-950/40 p-4">
                      <p className="text-xs font-semibold text-indigo-200 uppercase tracking-wider">Base Monthly Investment</p>
                      <div className="mt-2 flex items-baseline gap-2">
                        <p className="text-4xl font-black text-white">
                          $
                          {preset.tier === 'top' ? '1,995' : '995'}
                      ...
                      <Sheet>
                        ...
                        <SheetContent className={PUBLIC_SHEET_CLASS}>
                          ...
                          <div className="space-y-6">
                            ...
                            <div>
                              <h4 className="font-semibold text-white mb-3">
                                Monthly Core Services (${preset.tier === 'top' ? '1,995' : '995'}/month)
                              </h4>
                              <div className="space-y-2">
                                {preset.mustHaveMonthlyRetainers.map((id) => {
                                  const svc = CAMPAIGN_SERVICE_CATALOG.find(s => s.id === id)
                            ...
                            {preset.recommendedAddOns.length > 0 && (
                              <div>
                                <h4 className="font-semibold text-white mb-3">Recommended Add-ons (Optional)</h4>
                                <div className="space-y-2">
                                  {preset.recommendedAddOns.map((id) => {
                                    const svc = CAMPAIGN_SERVICE_CATALOG.find(s => s.id === id)
                                    return svc ? (
